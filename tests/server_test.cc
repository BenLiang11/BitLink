...
TEST_F(ServerTest, ConstructionInvokesStartAccept) {
  EXPECT_NO_THROW(server s(io_service, port));
}
...
TEST_F(ServerTest, HandleAcceptSuccessInvokesSessionStart) {
  ...
  struct SpySession : session {
    SpySession(boost::asio::io_service& io) : session(io), started(false) {}
    void start() override { started = true; }
    bool started;
  };

  auto* spy = new SpySession(io_service);
...
TEST_F(ServerTest, HandleAcceptErrorDeletesSession) {
  ...
  session* doomed = new session(io_service);
