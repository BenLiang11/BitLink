...
bool NginxConfigParser::Parse(std::istream* config_file, NginxConfig* config) {
  ...
  while (true) {
    ...
    if (token_type == TOKEN_TYPE_START) {
      ...
      // Allow only digits after listen
      if (!expect_listen_arg && token == "listen") {
        expect_listen_arg = true;
      } else if (expect_listen_arg) {
        for (char c : token) {
        ...
        expect_listen_arg = false;
        if (token_type == TOKEN_TYPE_ERROR) break;
      }
